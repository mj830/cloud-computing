...
$( document ).ready(function() {
  ...
  if (!('webkitSpeechRecognition' in window)) {
    ...
    recognition.onstart = function() {
      recognizing = true;
      showInfo('speak_now');
      start_img.src = '../static/gif/mic-animation.gif';
    };

    recognition.onerror = function(event) {
      if (event.error == 'no-speech') {
        start_img.src = '../static/gif/mic.gif';
        showInfo('no_speech');
        ignore_onend = true;
      }
    ...
    //   messageTextArea.value = linebreak(final_transcript + interim_transcript);
    // };
    recognition.onresult = function(event) {
    var interim_transcript = '';
    var final_transcript = '';
    ...
    // 从结果的起始位置开始迭代
    for (var i = event.resultIndex; i < event.results.length; ++i) {
        // 如果结果是最终结果，则将其添加到 final_transcript 中
        if (event.results[i].isFinal) {
            final_transcript += event.results[i][0].transcript;
        } else {
            // 如果结果是临时结果，则将其添加到 interim_transcript 中
            interim_transcript += event.results[i][0].transcript;
        }
    }

    // 将最终结果追加到 textarea 的末尾
    // 使用 linebreak() 函数处理文本格式
    messageTextArea.value += linebreak(final_transcript);
};
...
$("#start_button").click(function () {
  ...
  start_img.src = '../static/gif/mic-slash.gif';
  showInfo('allow');
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // 获取 language_select 元素
    var languageSelectDiv = document.getElementById("language_select");
    languageSelectDiv.style.display = "none";
    // // 如果是 Firefox 浏览器，则隐藏 language_select 元素
    // // if (isFirefox) {
    //     languageSelectDiv.style.display = "none";
    // // }
});
